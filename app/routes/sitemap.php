...
// Categories
foreach (DB::fetchAll("SELECT slug, name FROM categories WHERE status = 1") as $c) {
    echo '<url><loc>' . e($base . '/category/' . $c['slug']) . '</loc><priority>0.8</priority></url>' . "\n";
}

// Pages
foreach (DB::fetchAll("SELECT slug FROM pages WHERE status = 1") as $p) {
    echo '<url><loc>' . e($base . '/page/' . $p['slug']) . '</loc><priority>0.6</priority></url>' . "\n";
}

// News
foreach (DB::fetchAll(
    "SELECT slug, published_at FROM news WHERE status='published' AND published_at IS NOT NULL AND published_at <= NOW() ORDER BY published_at DESC LIMIT 5000"
) as $n) {
